    ...
    eachMessage: async ({ message }) => {
      if (message.value) {
        ...
        let image = await fetch(
          `http://localhost:8080/certificate/${uploadMessage.certificateId}/image`,
        );
        if (image.status !== 200) {
        ...
        let pdf = await fetch(
          `http://localhost:8080/certificate/${uploadMessage.certificateId}/pdf`,
        );
        if (pdf.status !== 200) {
